...
for(i in cities.to.run ) {
  ...
  formula <- death~cb+as.factor(thisage)+as.factor(dow)+ns(date,df=dfseas*length(unique(year)))  
  # MODEL 1, All ages
  data=data1
  tryCatch( source(paste(codepath,"modelhelper-cityspecific.R",sep="")), 
  ...
  formula <- death~cb+as.factor(dow)+ns(date,df=dfseas*length(unique(year)))  
  # MODEL 2, Age 65+
